...
@section('title', $chapter->title.' — Quizzes | GISBA Members')
@section('meta_description', 'Watch quiz videos for '.$chapter->title.'.')

@section('banner')
  <div class="d-flex justify-content-between align-items-center flex-wrap gap-2">
    <span><i class="bi bi-patch-question-fill me-2"></i>Quizzes</span>
...
<div class="rp-header">
  <div class="container">
    <div class="rp-meta">
      <span class="rp-type-label"><i class="bi bi-patch-question-fill"></i> Quizzes</span>
      <span class="rp-sep">•</span>
      <a href="{{ route('members.chapters.show', $chapter->slug) }}" class="rp-chapter-link">{{ $chapter->title }}</a>
      @if($chapter->resources->isNotEmpty())
        <span class="rp-count-badge">{{ $chapter->resources->count() }} {{ Str::plural('quiz', $chapter->resources->count()) }}</span>
      @endif
    </div>
  ...
  <div class="container">
    ...
    @if($chapter->resources->isEmpty())
      <div style="text-align:center;padding:72px 0;color:#9ca3af;">
        <i class="bi bi-patch-question" style="font-size:3rem;display:block;margin-bottom:16px;color:#d1d5db;"></i>
        <p style="font-size:14.5px;margin:0;">No quiz videos uploaded yet. Check back soon.</p>
      </div>
    @else
      <div class="row g-4">

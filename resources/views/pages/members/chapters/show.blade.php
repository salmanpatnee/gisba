...
@section('meta_description', $chapter->description ?? 'Access resources for ' . $chapter->title . ' in PMP Comprehensive
    Training Aligned with PMBOK 8th Edition.')
    ...
    <div class="d-flex justify-content-between align-items-center flex-wrap gap-2">
        ...
        <div class="d-flex gap-3 align-items-center">
            ...
            <form method="POST" action="{{ route('members.logout') }}" style="margin:0;">
                @csrf
                <button type="submit"
                    style="background:none;border:none;padding:0;color:inherit;cursor:pointer;font-size:inherit;"><i
...
@section('content')
    ...
    <style>
        ...
        .resource-row:hover .resource-row-action {
            color: var(--navy-mid);
        }

        .resource-row.is-done {
            border-left-color: #10b981;
            background: rgba(16, 185, 129, 0.05);
        }

        .resource-row.is-done:hover {
            border-left-color: #10b981;
        }

        .resource-row-action.is-done {
            color: #10b981;
        }
                ...
                <div class="col-12 col-lg-8">
                    <div class="resources-panel">
                        <div class="resources-panel-header">
                            <p class="resources-panel-title"><i class="bi bi-collection me-2"
                                    style="color:var(--accent);"></i>Chapter Resources</p>
                        </div>

                        @php($cat = $categoryProgress->get('tutorial'))
                        <a href="{{ route('members.chapters.tutorials', $chapter->slug) }}"
                    ...
                    <div class="chapter-nav">
                        ...
                        @if ($nextChapter)
                            <a href="{{ route('members.chapters.show', $nextChapter->slug) }}" class="chapter-nav-link"

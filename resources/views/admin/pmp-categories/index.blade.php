<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">PMP Categories</h2>
            <a href="{{ route('admin.pmp-categories.create') }}"
                ...
                <table class="min-w-full divide-y divide-gray-200 text-sm">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-left font-medium text-gray-500 uppercase tracking-wider">PMP Posts</th>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        <tr>
                            ...
                            <td class="px-6 py-4 text-gray-600">{{ $category->pmp_posts_count }} post(s)</td>
                            <td class="px-6 py-4 text-gray-600">{{ $category->created_at->format('M d, Y') }}</td>
                            <td class="px-6 py-4 text-right space-x-3">
                                <a href="{{ route('admin.pmp-categories.edit', $category) }}"
                                   class="text-indigo-600 hover:text-indigo-800 font-medium">Edit</a>
                                <form action="{{ route('admin.pmp-categories.destroy', $category) }}" method="POST" class="inline"
                                      onsubmit="return confirm('Delete this category? This cannot be undone if it has PMP posts.')">
                                    @csrf
                                    @method('DELETE')
                        ...
                        @empty
                        <tr>
                            <td colspan="4" class="px-6 py-10 text-center text-gray-500">No PMP categories yet.</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
